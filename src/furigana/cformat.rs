    ...
    T: AsRef<str>,
{
    ...
    pub fn merge_kanji_parts(mut self) -> Self {
        ...
        for (sub, is_kanji) in Furigana(str).gen_parser() {
            ...
            if (!is_kanji || !exit_undetailed) && !merge_buf.is_empty() {
            ...
            buf.push_str(i.as_kanji().unwrap());
...
mod test {
    ...
    fn test_merge_parts(src: &str, dst: &str) {
        ...
        let res = CodeFromatter::new(&furi).merge_kanji_parts().finish();
    ...
    fn test_merge_parts_lossy(src: &str, dst: &str) {
        let furi = Furigana(src);
        let res = CodeFromatter::new(&furi)
            .lossy()
            .merge_kanji_parts()
            .finish();
        assert_eq!(res.raw(), dst);
